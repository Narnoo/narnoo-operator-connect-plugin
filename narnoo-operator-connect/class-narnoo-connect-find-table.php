...
class Narnoo_Connect_Find_Table extends WP_List_Table {
    ...
    function get_columns() {
        return array(
            ...
            'connected'         => __( 'Connected', NARNOO_OPERATOR_CONNECT_I18N_DOMAIN )
        );
    }
    ...
    function get_current_page_data() {
        ...
        if ( ! is_null( $request ) ) {
                ...
                if( empty($list) ){
                    ...
                    try {
                        ...
                        $list = $request->findOperators( $current_page );
        ...
        if ( ! is_null( $list ) ) {
            ...
            foreach ( $list->data as $operators ) {
                ...
                $item['location']           = $operators->location;
                $item['narnoo_id']          = $operators->details->id;
                $item['type']               = $operators->details->type;
                $item['connected']          = $operators->details->connected;
                $data['items'][]            = $item;
            }
        }
        return $data;
